...
export default function TcgPokemon() {
  const { cards, isLoading, totalValue, totalCost, totalGain, totalGainPercent, createCard, updateCard, deleteCard } = useTcgCards('pokemon');
  ...
  const uniqueSets = useMemo(() => {
    ...
    cards.forEach(c => { if (c.set_code) sets.add(c.set_code); });
  ...
  const filtered = cards.filter((c) => {
    ...
    const matchSet = setFilter === 'all' || c.set_code === setFilter;
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <h1 className="text-3xl font-display font-bold">Pokémon TCG</h1>
            <p className="text-muted-foreground">Collezione carte Pokémon</p>
          </div>
          <Dialog open={open} onOpenChange={(o) => { if (!o) resetDialog(); else setOpen(true); }}>
            ...
            <DialogContent className="max-w-7xl max-h-[90vh] overflow-y-auto">
              <DialogHeader><DialogTitle>Aggiungi carta Pokémon</DialogTitle></DialogHeader>
                ...
                <div>
                  ...
                  <div className="flex gap-2 mt-1">
                    <Input
                      ...
                      placeholder="es. Charizard, Pikachu..."
                ...
                {selected && (
                  <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="flex items-center gap-3 p-3 bg-secondary rounded-lg">
                      {selected.image && <img src={selected.image} alt={selected.name} className="w-12 h-16 object-cover rounded" />}
                      <div>
        ...
        <div className="glass rounded-xl divide-y divide-border">
          <div className="grid grid-cols-[auto_1fr_auto_auto_auto_auto_auto_auto] gap-4 px-4 py-2 text-xs text-muted-foreground font-medium">
            <span className="w-8 block"></span><span>Carta</span><span>Cond.</span><span>Lingua</span>
            <span className="text-right">Qta</span><span className="text-right">Acquisto</span>
            <span className="text-right">Attuale</span><span className="w-[4.5rem] block"></span>
          </div>
          {isLoading ? (
              ...
              return (
                <div key={card.id} className="grid grid-cols-[auto_1fr_auto_auto_auto_auto_auto_auto] gap-4 items-center px-4 py-3">
                  ...
                  <div>
                    ...
                    {card.set_code && <p className="text-xs text-muted-foreground">{card.set_code}{card.collector_number != null ? ` #${card.collector_number}` : ''}</p>}
                  </div>
              ...
              <form onSubmit={handleEditSubmit} className="space-y-4">
                <div className="flex items-center gap-3 p-3 bg-secondary rounded-lg">
                  ...
                  <div>
                    ...
                    {editingCard.set_code && <p className="text-sm text-muted-foreground">{editingCard.set_code}{editingCard.collector_number != null ? ` #${editingCard.collector_number}` : ''}</p>}
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
