...
export default function FumoCBD() {
  ...
  const [entries, setEntries] = useState<CBDEntry[]>([]);
  ...
  const [editingEntry, setEditingEntry] = useState<CBDEntry | null>(null);
  ...
  const calcolareCampi = (entry: CBDEntry): CBDEntry => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const entriesWithCalculated = (data as unknown as CBDEntry[]).map(calcolareCampi);
      setEntries(entriesWithCalculated);
    } catch (error) {
