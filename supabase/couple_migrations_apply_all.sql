...
-- Ordine: couple_connections creata per prima perché referenziata da
--         helper functions, RLS policies e foreign keys successive.
-- =============================================================================

-- ============================================================
-- STEP 1: add_couple_expenses_permission
-- ============================================================
...
-- ============================================================
-- STEP 2: couple_code su profiles
-- ============================================================
...
BEGIN
  LOOP
    ...
    END LOOP;
    SELECT EXISTS (SELECT 1 FROM public.profiles WHERE couple_code = v_code) INTO v_exists;
...
CREATE TRIGGER trg_set_couple_code
  ...
  FOR EACH ROW EXECUTE FUNCTION public.set_couple_code();
...
BEGIN
  FOR r IN SELECT id FROM public.profiles WHERE couple_code IS NULL LOOP
    UPDATE public.profiles SET couple_code = public.generate_couple_code() WHERE id = r.id;
  END LOOP;
END;
$$;

ALTER TABLE public.profiles ALTER COLUMN couple_code SET NOT NULL;
...
-- ============================================================
-- STEP 3: couple_connections (PRIMA di tutto ciò che la usa)
-- ============================================================
...
CREATE UNIQUE INDEX couple_connections_user_a_active
  ON public.couple_connections (user_a) WHERE revoked_at IS NULL;

DROP INDEX IF EXISTS couple_connections_user_b_active;
CREATE UNIQUE INDEX couple_connections_user_b_active
  ON public.couple_connections (user_b) WHERE revoked_at IS NULL;
...
-- ============================================================
-- STEP 4: helper functions (couple_connections ora esiste)
-- ============================================================
...
BEGIN
  RETURN EXISTS (
    ...
    WHERE cc.id = p_connection_id AND cc.revoked_at IS NULL
...
BEGIN
  IF NOT public.is_couple_expenses_enabled() THEN RETURN NULL; END IF;
...
-- ============================================================
-- STEP 5: RLS su couple_connections
-- ============================================================
...
-- ============================================================
-- STEP 6: couple_connection_requests
-- ============================================================
...
CREATE POLICY "ccr_insert" ON public.couple_connection_requests
  FOR INSERT WITH CHECK (
    ...
    AND (
      SELECT COUNT(*) FROM public.couple_connection_requests existing
...
CREATE POLICY "ccr_update_sender_cancel" ON public.couple_connection_requests
  FOR UPDATE USING (
    public.is_couple_expenses_enabled() AND sender_id = auth.uid() AND status = 'pending'
...
CREATE POLICY "ccr_update_receiver_reject" ON public.couple_connection_requests
  FOR UPDATE USING (
    public.is_couple_expenses_enabled() AND receiver_id = auth.uid() AND status = 'pending'
  ) WITH CHECK (status = 'rejected');

-- ============================================================
-- STEP 7: accept_couple_request RPC
-- ============================================================
...
CREATE TRIGGER trg_se_updated_at
  BEFORE UPDATE ON public.shared_expenses FOR EACH ROW EXECUTE FUNCTION public.touch_updated_at_se();
...
CREATE TRIGGER trg_tx_soft_delete_cascade_shared
  AFTER UPDATE ON public.transactions FOR EACH ROW EXECUTE FUNCTION public.on_tx_soft_delete_remove_shared_expense();
...
CREATE POLICY "se_insert" ON public.shared_expenses
  FOR INSERT WITH CHECK (
    ...
    AND EXISTS (
      ...
      WHERE t.id = original_tx_id AND t.user_id = auth.uid()
...
CREATE POLICY "cb_insert" ON public.couple_budgets
  FOR INSERT WITH CHECK (public.is_couple_expenses_enabled() AND public.is_in_connection(connection_id) AND created_by = auth.uid());

DROP POLICY IF EXISTS "cb_update" ON public.couple_budgets;
CREATE POLICY "cb_update" ON public.couple_budgets
  FOR UPDATE USING (public.is_couple_expenses_enabled() AND public.is_in_connection(connection_id));

DROP POLICY IF EXISTS "cb_delete" ON public.couple_budgets;
CREATE POLICY "cb_delete" ON public.couple_budgets
  FOR DELETE USING (public.is_couple_expenses_enabled() AND public.is_in_connection(connection_id));

-- ============================================================
-- STEP 10: couple_audit_log
-- ============================================================
...
CREATE OR REPLACE FUNCTION public.write_couple_audit_log(
  p_connection_id UUID, p_action TEXT, p_target_id UUID DEFAULT NULL, p_metadata JSONB DEFAULT '{}'::jsonb
)
RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
...
-- ============================================================
-- STEP 11: aggiorna RPCs con audit log + trigger di audit
-- ============================================================
...
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  PERFORM public.write_couple_audit_log(NULL, 'request_sent', NEW.id, jsonb_build_object('sender_id', NEW.sender_id, 'receiver_id', NEW.receiver_id));
  RETURN NEW;
END;
$$;
DROP TRIGGER IF EXISTS trg_audit_request_sent ON public.couple_connection_requests;
CREATE TRIGGER trg_audit_request_sent
  AFTER INSERT ON public.couple_connection_requests FOR EACH ROW EXECUTE FUNCTION public.audit_couple_request_sent();
...
CREATE TRIGGER trg_audit_request_status
  AFTER UPDATE ON public.couple_connection_requests FOR EACH ROW EXECUTE FUNCTION public.audit_couple_request_status_change();
...
CREATE TRIGGER trg_audit_connection_revoked
  AFTER UPDATE ON public.couple_connections FOR EACH ROW EXECUTE FUNCTION public.audit_couple_connection_revoked();
...
CREATE TRIGGER trg_audit_shared_expense_created
  AFTER INSERT ON public.shared_expenses FOR EACH ROW EXECUTE FUNCTION public.audit_shared_expense_created();
