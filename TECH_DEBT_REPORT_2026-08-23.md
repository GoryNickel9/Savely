...
2. **TD-005**: ✅ chiuso il 23/08/2026 (III/IV). Libreria consolidata: `Fumetti.tsx` 415→21 e `Libri.tsx` 437→21 righe, ora wrapper di `components/libreria/LibraryPage` (config: categoria, funzione di ricerca, label i18n) con la ricerca Google Books estratta in `lib/googleBooks.ts`. **Manga resta volutamente separato** (inserimento manuale con range di volumi e dialog di modifica su tutti i campi — UX genuinamente diverse). **Poker: nessun consolidamento, decisione motivata dall'analisi**: `PokerNextCut` non è un CRUD (pagina di calcolo su budgets/transazioni/spese), e `PokerRakeback` vs `PokerHourlyEarnings` differiscono per 628 righe su ~500 — modello dati diverso (CRUD per data vs upsert mensile con campi EV e deal da `poker_next_cut`). Un componente generico richiederebbe un framework a config di campi: complessità superiore alla duplicazione. Debito residuo accettato.
...
5. **TD-009**: ✅ chiuso il 23/08/2026 (IV). `useSupabaseData` — l'unico hook con fetch server manuale (useState/useEffect + ref anticarro-concurrency) — migrato a React Query con API pubblica invariata (`{data, loading, reload}`): i 5 consumer (FumoCrudPage ×3, PokerHourlyEarnings, PokerRakeback) non cambiano; `retry: false` e `refetchOnWindowFocus: false` preservano il comportamento precedente. Test aggiornati con wrapper QueryClient. **Rimangono volutamente manuali**: `usePermissions` (cache localStorage per gating istantaneo dei permessi, design deliberato), `useCardTraderSearch` (pattern di ricerca imperativa, non cache di query), `useRecurringCandidates` (derived state su hook già React Query), `useFireCalculatorParams` (persistenza parametri, non fetching), `useDialogManager`/`use-toast`/`use-mobile` (non sono data fetching).
